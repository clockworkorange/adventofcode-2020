...
class Day2Test extends Specification {
    ...
    void 'Day2 split password policy and password'() {
        when:
        List<List<String>> result = exampleInput.collect { it.split(': ') }
    ...
    void 'Day2 get occurrences and letter from password policy'() {
        given:
        List<List<String>> inputSplitted = exampleInput.collect { it.split(': ') }

        when:
        List<List<String>> result = inputSplitted.collect { it.first().split(' ') + it.last() }
    ...
    void 'Day 2 get min & max occurrences'() {
        given:
        List<List<String>> inputSplitted = exampleInput.collect { input ->
            List<List<String>> policyAndPassword = input.split(': ')
            policyAndPassword.first().split(' ') + policyAndPassword.last()
        }

        when:
        List<List<String>> result = inputSplitted.collect { input ->
            input[0].split('-').collect { it.toInteger() } + input[1] + input[2]
        }
    ...
    void 'Day 2 validate part one'() {
        given:
        List<List<String>> inputSplitted = exampleInput.collect { input ->
            List<List<String>> policyAndPassword = input.split(': ')
            List<List<String>> policySplittedAndPassword = policyAndPassword.first().split(' ') + policyAndPassword.last()
            policySplittedAndPassword[0].split('-').collect { it.toInteger() } +
                policySplittedAndPassword[1] +
                policySplittedAndPassword[2]
        }

        when:
        Integer result = inputSplitted.count { input ->
            Integer occurrences = input[3].count(input[2])
            occurrences >= input[0] && occurrences <= input[1]
        }
    ...
    void 'Day 2 validate part two'() {
        ...
        Integer result = inputSplitted.count { input ->
            ...
            Integer firstPosition = input[0] - 1
            Integer secondPosition = input[1] - 1
    ...
    private static final exampleInput = [
        '1-3 a: abcde',
        '1-3 b: cdefg',
        '2-9 c: ccccccccc',
    ]

    private static final puzzleInput = [
        '2-4 r: prrmspx',
        '5-6 p: hpzplphxb',
        '5-8 t: ttttbtttttc',
        '1-6 k: kkkkkk',
